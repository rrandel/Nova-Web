...
const MenuToggle = () => {
  ...
  return (
    <>
      ...
      <div id="menuItems" className={`fixed top-0 right-0 h-full w-64 bg-darkMode-primaryBackground text-white shadow-lg transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'} md:hidden`}>
        <nav className="flex flex-col p-6 space-y-4">
          ...
          <button onClick={toggleMenu} className="mt-auto p-2 text-lg font-semibold rounded bg-gray-700 hover:bg-gray-600"> Close </button>
        </nav>
      </div>
    </>
  )
